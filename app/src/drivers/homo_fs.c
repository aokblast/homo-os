...
static int
homo_fs_mount(struct fs_mount_t *mount)
{
  ...
  return (fs->fs = homo_fs_deserialize(backend->base_addr, backend->size)) == NULL;
}

static int
homo_fs_unmount(struct fs_mount_t *mount) {
  struct homo_fs_filesystem_param *fs = mount->fs_data;
  homo_fs_free(fs->fs);
  return 0;
}
...
static int
homo_fs_open(struct fs_file_t *filp, const char *fs_path, fs_mode_t flags) {
...
static ssize_t
homo_fs_read(struct fs_file_t *filp, void *dest, size_t nbytes) {
  ...
  return homo_fs_entry_file_read_offset(param->fent, (uint8_t *)dest, nbytes, offset);
}
...
int homo_fs_readdir(struct fs_dir_t *dirp, struct fs_dirent *entry) {
  ...
  entry->type = homo_fs_entry_get_type(fent) == FS_DIR ? FS_DIR_ENTRY_DIR : FS_DIR_ENTRY_FILE;
  entry->size = homo_fs_entry_get_type(fent) == FS_DIR ? 0 : homo_fs_entry_file_get_size(fent);
  strncpy(entry->name, homo_fs_entry_get_name(fent), MAX_FILE_NAME);
  dirp->dirp = homo_fs_entry_next_file(fent);
  return 0;
}

int homo_fs_closedir(struct fs_dir_t *dirp __unused) {
  return 0;
}

int homo_fs_stat(struct fs_mount_t *mountp, const char *path, struct fs_dirent *entry) {
